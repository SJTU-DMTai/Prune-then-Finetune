...
#  read pr ema lr <<< "$(python src/tsfm/select_hp.py --prune_ratio_per_epoch $pr --filename $filename --prune_ema $ema --learning_rate 1e-4 1e-5 1e-6 1e-7 1e-8 1e-9)"
  filename=./logs/tsfm/$data'_'$model_name'_'$model_size'_'$mode'_L'$L'_H'$pred_len'_P'$P'_prune_pr{}_ema{}_lr{}'.log
...
#  if [ ! -e $filename ] || [ ! -n "$(grep 'mse:' $filename)" ]; then
  echo $filename
  CUDA_VISIBLE_DEVICES=$[i+7] python -u src/tsfm/run.py \
