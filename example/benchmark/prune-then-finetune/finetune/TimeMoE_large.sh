...
for pred_len in 96
do
  torchrun --nproc_per_node=8 src/tsfm/run.py --use_multi_gpu --finetune_epochs 1 \
    ...
    --batch_size 32 --macro_batch_size 64 >> ./logs/tsfm/$data'_'$model_name'_'$model_size'_L'$seq_len'_H'$pred_len'_prune_raw_'$thres'_pr'$pr'_ema'$ema'_balance_lr'$lr.log 2>&1
done
done
done
done
